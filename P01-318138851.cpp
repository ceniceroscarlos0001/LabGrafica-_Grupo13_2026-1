...
const glm::vec3 ROJO = glm::vec3(1.0f, 0.0f, 0.0f);
const glm::vec3 VERDE = glm::vec3(0.0f, 1.0f, 0.0f);
const glm::vec3 AZUL = glm::vec3(0.0f, 0.0f, 1.0f);
const int var = 0;
...
inline void glClearColor(const glm::vec3& color, float alpha) {
 ...
 //Llamada a las funciones creadas antes del main
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		if (tiempo - ultimoCambio >= 2.0)
		{
			int nuevaFase;
			do {
				nuevaFase = std::rand() % 3;
			} while (nuevaFase == fase);
		...
		if (fase == 0) {
			glClearColor(ROJO, 1.0f);
		}
		else if (fase == 1) {
			glClearColor(VERDE, 1.0f);
		}
		else {
			glClearColor(AZUL, 1.0f);
		}
